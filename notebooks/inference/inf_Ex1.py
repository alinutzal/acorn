...
def inference(model_mm, model_gnn, device):
    ...
    #for batch_idx, batch in enumerate(model_gnn.valset):
        ...
        print("MM: ", ((end - start),(end_cpu - start_cpu), gpu_time))
        ...
        end_cpu = time.process_time()
        running_time.extend(((end - start),(end_cpu - start_cpu), gpu_time))
        print("GNN: ", ((end - start),(end_cpu - start_cpu), gpu_time))
        ...
        end_cpu = time.process_time()
        running_time.extend(((end - start),(end_cpu - start_cpu), gpu_time))
        print("Tracking: ", ((end - start),(end_cpu - start_cpu), gpu_time))
...
if __name__ == "__main__":
    ...
    inference(model_mm, model_gnn, device)
    end = time.time()
    end_cpu = time.process_time()
    if device == 'cuda':
        ender.record()
        torch.cuda.synchronize()
        gpu_time = starter.elapsed_time(ender)/1000.0     
    print("Total: ",((end - start),(end_cpu - start_cpu),gpu_time)) 