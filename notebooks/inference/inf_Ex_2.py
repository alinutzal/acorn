...
def inference(model_ml, model_gnn, dataloader, device):
    graphs = []
    for batch_idx, batch in enumerate(dataloader):
        batch = batch.to(device)    
        out = model_ml.shared_evaluation(batch,batch_idx)
...
if __name__ == "__main__":
    ...
    #########################################
    config_ml = yaml.load(open(os.environ['HOME']+'/acorn/examples/Example_2/metric_learning_infer.yaml'), Loader=yaml.FullLoader)
    print(config_ml)
    config_gnn = yaml.load(open(os.environ['HOME']+'/acorn/examples/Example_1/gnn_infer.yaml'), Loader=yaml.FullLoader)
    config_tbe = yaml.safe_load(open(os.environ['HOME']+"/acorn/examples/Example_2/track_building_eval.yaml", "r"))
    model_ml = MetricLearning.load_from_checkpoint(config_ml['stage_dir']+'artifacts/best-v2.ckpt',config=config_ml)    
    model_ml.setup('predict')
    model_ml.to(device)
    dataloaders = model_ml.predict_dataloader()   
    model_gnn = InteractionGNN.load_from_checkpoint(config_gnn['stage_dir']+'artifacts/best-v3.ckpt',config=config_gnn)    
    model_gnn.setup('predict')
    model_gnn.to(device)
    #dataloaders = model_gnn.predict_dataloader()
    
    inference(model_ml, model_gnn, dataloaders[2], device)
